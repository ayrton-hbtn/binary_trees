...
int binary_tree_is_bst_valid(binary_tree_t *tree, int nodeVal, int lessThan)
{
	if (tree != NULL)
	{
		if (lessThan)
		{
			if (tree->n >= nodeVal)
				return (0);
			return (binary_tree_is_bst_valid(tree->left, nodeVal, lessThan) &&
					binary_tree_is_bst_valid(tree->right, nodeVal, lessThan));
		}
		else
		{
			if (tree->n <= nodeVal)
...
int binary_tree_is_bst(const binary_tree_t *tree)
{
	if (tree != NULL)
	{
		if (binary_tree_is_bst_valid(tree->left, tree->n, 1) &&
				binary_tree_is_bst_valid(tree->right, tree->n, 0))
